...
// Access vditor internals at runtime (not available as npm imports)
const getI18n = () => (window as any).vditor?.vditor?.options?.i18n || {}
const updateHotkeyTip = (hotkey: string): string => {
  const isMac = navigator.platform.toLowerCase().includes('mac')
  if (isMac) return hotkey
  return hotkey.replace('⌘', 'Ctrl').replace('⇧', 'Shift').replace('⌥', 'Alt')
}
...
export function fixTableIr() {
  ...
  >
   <button
      type="button"
      aria-label="${i18nData.alignLeft || 'Align Left'}<${updateHotkeyTip('⇧⌘L')}>"
      data-type="left"
    ...
    ><button
      type="button"
      aria-label="${i18nData.alignCenter || 'Align Center'}<${updateHotkeyTip('⇧⌘C')}>"
      data-type="center"
    ...
    ><button
      type="button"
      aria-label="${i18nData.alignRight || 'Align Right'}<${updateHotkeyTip('⇧⌘R')}>"
      data-type="right"
      class="vditor-icon vditor-tooltipped vditor-tooltipped__n"
    >
    ...
    ><button
      type="button"
      aria-label="${i18nData.insertRowAbove || 'Insert Row Above'}<${updateHotkeyTip('⇧⌘F')}>"
    ...
    ><button
      type="button"
      aria-label="${i18nData.insertRowBelow || 'Insert Row Below'}<${updateHotkeyTip('⌘=')}>"
    ...
    ><button
      type="button"
      aria-label="${i18nData.insertColumnLeft || 'Insert Column Left'}<${updateHotkeyTip('⇧⌘G')}>"
    ...
    ><button
      type="button"
      aria-label="${i18nData.insertColumnRight || 'Insert Column Right'}<${updateHotkeyTip('⇧⌘=')}>"
    ...
    ><button
      type="button"
      aria-label="${i18nData['delete-row'] || 'Delete Row'}<${updateHotkeyTip('⌘-')}>"
      data-type="deleteRow"
      class="vditor-icon vditor-tooltipped vditor-tooltipped__n"
    >
    ...
    ><button
      type="button"
      aria-label="${i18nData['delete-column'] || 'Delete Column'}<${updateHotkeyTip('⇧⌘-')}>"
